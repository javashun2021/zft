...
def handle_text(update, context):
    ...
    if message.startswith("转单"):
        ...
        if match:
            ...
            try:
                url = f"http://127.0.0.1:5000/query?orderNo={order_nos}"
                response = requests.get(url)
                data = response.json()
                # 提取 data 信息
    ...
    elif message.startswith("查多单"):
        # 提取订单号（假设格式是 查询 + 空格 + 订单号）
        match = re.search(r'查多单\s*([A-Z0-9,]+)', message)
        if match:
            order_nos = match.group(1)
            update.message.reply_text(f"✅ 收到多查询订单号：{order_nos}，正在处理...")

            try:
                url = f"http://127.0.0.1:5000/quersy?orderNos={order_nos}"
                response = requests.get(url)
                data = response.json()
                # 提取 data 信息
                results = data.get("results", [])
